...
//        } 
        ...
        debug->print("I2C Error read current failed:");
        debug->println(error);
        return 0;
      }
    };
    ...
    uint8_t powerDown(bool down) {
      ...
      if ( !down ) {
        ...
        if ( error == I2C_OK ) {
          if ( value == config ) {
            debug->print("Config confirmed as ");
            debug->println(config);
          } else {
            debug->print("Config failed requested: ");
            debug->print(config);
            debug->print(" set:");
            debug->println(value);
          }
        } else {
          debug->print("Failed to read back confg register ");
          debug->println(error);
        }
      }
...
private:
  ...
  int16_t readRegister(int8_t r) {
      ...
      error = Wire.endTransmission();
      if ( error != 0) {
        debug->print("I2C Failed to select register");
        debug->println(r, HEX);
        return 0;
      }
      if ( Wire.requestFrom(address, 2) == 2 ) {
        ...
        debug->println("I2C Error read current incomplete");
